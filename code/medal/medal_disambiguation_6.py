...
class config:
    ...
    TRAIN_FILE = '../../data/AD_medal/csv_files/train_pairs.csv'
    VAL_FILE = '../../data/AD_medal/csv_files/dev_pairs.csv'
    TEST_FILE = '../../data/AD_medal/csv_files/test_pairs.csv'
    EXTERNAL_FILE = '../../data/AD_medal/csv_files/external_pairs.csv'

    ANCHOR_FILE = '../../data/AD_medal/anchor_sent.csv'
    ...
    TOKENIZER = AutoTokenizer.from_pretrained("dmis-lab/biobert-v1.1")
    # TOKENIZER = AutoTokenizer.from_pretrained("xhlu/electra-medal", return_token_type_ids=False)
    ALPHA = 0.5
    TRAIN_BATCH_SIZE = 32
    VALID_BATCH_SIZE = 32
...
def run_k_fold(fold_id):
    ...
    # df_train = get_sample_dataframe(df_train)
    # df_val = get_sample_dataframe(df_val)
    # df_test = get_sample_dataframe(df_test)
...
if __name__ == '__main__':
    ...
    with open(details_file_path, 'a') as file:
        file.write('File Name: MEDAL - Disambiguation 6')
    ...
    try:
        ...
        with open(details_file_path, 'a') as file:
            file.write(
                f'\nNo. of folds:  {len(f)} | Final of valid score:  {np.mean(f)}')
