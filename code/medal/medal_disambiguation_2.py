#  code with multiple positive negative pairs and triplet loss. considers all the negative expansions. when getting the performance metrics the absolute value of distances is considered. not the cosie
...
class config:
    ...
    TRAIN_FILE = '../../data/AD_medal/csv_files/train.csv'
    VAL_FILE = '../../data/AD_medal/csv_files/dev.csv'
    TEST_FILE = '../../data/AD_medal/csv_files/test.csv'

    ALL_ANCHOR_FILE = '../../data/AD_medal/all_anchor_sent.csv'
    SAVE_DIR = '../../models/'
    DETAILS_FILE = 'details.txt'
    DICTIONARY = json.load(open('../../data/AD_medal/medal_dictionary.json'))
    ...
    ACRONYM_LIST_PATH = '../../data/AD_medal/random_acronyms.pkl'
    with open(ACRONYM_LIST_PATH, 'rb') as f:
        ACRONYMS = pickle.load(f)

    ACRONYMS = ACRONYMS[:100]
    TRAIN_SAMPLE_SIZE = 24000
...
class Dataset:
    ...
    def process_data_(self, text, acronym, expansion, tokenizer, max_len, other_expansion, anchor_sentence, mode):
        ...
        if mode == 'train':
            ...
            #  list of negative  sentences -- acornym in the sample sentence is replaced by the incorrect expansions
            ...
            positive_token_ids = tokenizer(positive_sentence, padding="max_length", max_length=config.MAX_LEN,
                                           return_tensors="pt")
            # in the validation stage all expanions are considered as other. therefore append the correct expansion as well
            other.append(expansion)

            #  list of negative  sentences -- acornym in the sample sentence is replaced by the all expansions
    ...
    def __getitem__(self, item):
        ...
        if len(data['other_ids']) > config.max_expansions or len(
                data['other_token_type']) > config.max_expansions or len(data['other_mask']) > config.max_expansions:
...
class BertAD(nn.Module):
    ...
    def forward(self, anchor_token_ids, anchor_mask, anchor_token_type,
        ...
        for i in range(max_lengths):
            ...
            if i == 0:
                expansions_embeddings = output
            else:
                expansions_embeddings = torch.cat((expansions_embeddings, output), dim=1)
            # in the validation and test stages, the anchor sentece is the positive sample with  no replacement

        if mode == 'train':
            anchor_embedding = self.forward_one(anchor_token_ids, anchor_mask, anchor_token_type)
            positive_embedding = self.forward_one(token_ids, token_mask, token_type)
            return anchor_embedding, positive_embedding, expansions_embeddings
        else:
            anchor_embedding = self.forward_one(token_ids, token_mask, token_type)
            return anchor_embedding, expansions_embeddings
...
def modified_triplet_loss(max_length, anchor, positive, negative, alpha=0.4, device='cuda:0'):
    """
    Implementation of the triplet loss function
    Arguments:
    y_true -- true labels, required when you define a loss in Keras, you don't need it in this function.
    ...
    neg_distances = []
    for i in range(n):
        neg_dist = torch.sum((anchor - negative[:, i, :]).pow(2), axis=1)
        neg_dist = torch.unsqueeze(neg_dist, dim=1)
        if i == 0:
            neg_distances = neg_dist
        else:
            neg_distances = torch.cat((neg_distances, neg_dist), dim=1)

    min_neg_distance = torch.min(neg_distances, 1)

    # compute loss
    basic_loss = pos_dist - min_neg_distance[0] + alpha
    # basic_loss = n * pos_dist - neg_distances + n * alpha
    loss = torch.max(basic_loss, torch.tensor([0], device=device).float())

    return loss, pos_dist, min_neg_distance[0]
...
def train_fn(data_loader, model, optimizer, device):
    ...
    for anchor_token_ids, ids, d, lengths, correct_expansion, possible_expansions in data_loader:
        ...
        max_length = max_lenghts.to(device)
        ...
        model.zero_grad()
        anchor_embedding, positive_embedding, negative_embedding = model(anchor_ids, anchor_attention_mask,
        ...
        loss, pos_dist, neg_dist = modified_triplet_loss(max_length, anchor_embedding, positive_embedding,
                                                         negative_embedding, device=device)
...
def eval_fn(data_loader, model, device):
    ...
    for anchor_token_ids, ids, d, lengths, correct_expansion, possible_expansions in data_loader:
        ...
        with torch.no_grad():
            ...
            positive_embedding, expansion_embeddings = model(anchor_ids, anchor_attention_mask, anchor_token_type_ids,
                                                             positive_ids, positive_attention_mask,
                                                             positive_token_type_ids,
                                                             negative_ids, negative_attention_mask,
                                                             negative_token_type_ids,
                                                             mode='test', max_lengths=max_lenghts)
            min_indices = compute_distances_for_valid(expansion_embeddings, positive_embedding, lengths, max_lenghts)
...
def run(df_train, df_val, df_test, fold, df_anchor_sent):
    train_dataset = Dataset(
        ...
        other_expansion=df_train.other_expansion.values,
...
def get_sample_dataframe(df, sample_size):
    df = df[~df['acronym_'].isin(config.ignore_acronyms)]
    df = df[df['acronym_'].isin(config.ACRONYMS)]
    print(config.ACRONYMS)
    print(config.ignore_acronyms)

    # sorted_df = df.sort_values("acronym_")
...
def run_k_fold(fold_id):
    ...
    df_test = set_label(df_test)

    df_train = get_sample_dataframe(df_train, config.TRAIN_SAMPLE_SIZE)
    df_val = get_sample_dataframe(df_val, config.VALID_SAMPLE_SIZE)
    df_test = get_sample_dataframe(df_test, config.TEST_SAMPLE_SIZE)
...
if __name__ == '__main__':
    ...
    with open(details_file_path, 'a') as file:
        file.write('File Name: MEDAL - Disambiguation 2')
        file.write(f'\nStart time:{timestamp} ')
        file.write(
            '\nConsiders anchor positive all negatives at a time. Changed how the input was processed.')
        file.write(
            '\nIn the training stage, anchor sentence: respective anchor sentence with the acronym. expansion is not in the sentence. | positive sentence: sample sentence with the acronym replaced with correct expansion. | Negative sentences: sample sentence with the acronym replaced with  incorrect expansions')
        file.write(
        ...
        file.write(
            f'\nNo. of epochs:  {config.EPOCHS} |  Batch size :{config.TRAIN_BATCH_SIZE} | Kfold :{config.KFOLD} | Max len  :{config.MAX_LEN} | Learning Rate :{config.LEARNING_RATE} | Dropout :{config.DROPOUT} | Freeze layer count :{config.freeze_layer_count} | ALPHA for triplet loss :{config.ALPHA} | Includes external data for training :{config.INCLUDE_EXT}\n')

    try:
        f0 = run_k_fold(0)
        f = [f0]
        print(f'\nFinal valid F score:. {np.mean(f)}')
        end_timestamp = datetime.now().strftime(config.time_format)
        with open(details_file_path, 'a') as file:
            file.write(
                f'\nNo. of folds:  {len(f)} | Final valid F score:  {np.mean(f)}')
