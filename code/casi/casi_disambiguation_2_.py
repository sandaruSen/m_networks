...
class config:
    ...
    ALL_ANCHOR_FILE = '../../data/AD_CASI/all_anchor_sent.csv'
    SAVE_DIR = '../../models/'
...
def run_k_fold(fold_id):
    ...
    df_train = pd.read_csv(config.TRAIN_FILE)[:10]
    df_val = pd.read_csv(config.VAL_FILE)[:10]
    df_test = pd.read_csv(config.TEST_FILE)[:10]
...
if __name__ == '__main__':
    ...
    with open(details_file_path, 'a') as file:
        file.write('File Name: Disambiguation 2 ******** with processing changed for CASI AD data:'+config.MODEL_NAME)
